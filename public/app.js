...
async function render(result, run) {
  ...
  const verification = $('#verification'); verification.textContent = result.verification === 'live-triggered' ? `已受控提交 Agent Compose · ${result.live?.triggerId?.slice(0, 8) || 'accepted'}` : result.verification === 'server-validated' ? '服务器联调已通过' : '稳定回放'; verification.className = `verification ${result.verification}`;
...
async function replay(id, userText, live = null) { const response = await fetch('/api/replay', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ caseId: id }) }); if (!response.ok) throw new Error('replay_failed'); const result = await response.json(); if (live) { result.verification = 'live-triggered'; result.live = live; result.reply = `已受控提交至 ${live.project}/${live.agent}；${result.reply} 下方为不含原始数据的可重复证据回放，实际执行状态请在 Agent Compose UI 审计。`; } appendMessage('user', userText); appendMessage('bot', result.reply); const run = ++state.run; await render(result, run); }
async function triggerLive(caseId, message) { const response = await fetch('/api/live', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ caseId, message }) }); const result = await response.json(); if (response.status === 409) return { replayOnly: true, message: result.message }; if (!response.ok) throw new Error(result.error || 'live_trigger_failed'); return result; }
...
async function boot() {
  ...
  $('#mode').textContent = state.config.live?.ready ? 'HYBRID · 受控触发' : `${state.config.mode.toUpperCase()} MODE`; renderCases(); renderMetrics(); releaseMode();
  ...
  $('#chat-form').addEventListener('submit', async (event) => { event.preventDefault(); const input = $('#prompt'); const message = input.value.trim(); if (!message) return; const prepared = message === state.preparedText && state.preparedCaseId ? state.preparedCaseId : null; const id = prepared || caseForMessage(message); input.value = ''; clearPrepared(); try { const live = prepared && state.config.live?.ready ? await triggerLive(prepared, message) : null; if (live?.replayOnly) appendMessage('bot', `${live.message} 已按案例回放，不会触发 Agent Compose。`); await replay(id, message, live?.replayOnly ? null : live); } catch (error) { appendMessage('bot', `受控触发或回放失败：${error.message}。浏览器未直连 Agent Compose 或 OctoBus。`); } });
}
