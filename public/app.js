...
async function triggerLive(caseId, message) { const response = await fetch('/api/live', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ caseId, message }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'live_trigger_failed'); return result; }
